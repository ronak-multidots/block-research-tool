/**
 * Icon library shown above (or beside) a tab label.
 *
 * The geometry is duplicated in `includes/class-icons.php` because the front end is
 * rendered by PHP. Both lists are asserted against the enum in `src/tab/block.json`,
 * so a name added to one and forgotten in the other fails the test suite.
 */
...
export function iconNames() {
	return [ ICON_NONE, ...Object.keys( ICON_PATHS ) ];
}

/**
 * Human readable name for each icon, used by the picker in the sidebar.
 *
...
export function iconLabel( name ) {
	const labels = {
		[ ICON_NONE ]: __( 'No icon', 'tabs-block' ),
		target: __( 'Target', 'tabs-block' ),
		bolt: __( 'Lightning bolt', 'tabs-block' ),
		compass: __( 'Compass', 'tabs-block' ),
		rocket: __( 'Rocket', 'tabs-block' ),
		star: __( 'Star', 'tabs-block' ),
		heart: __( 'Heart', 'tabs-block' ),
		shield: __( 'Shield', 'tabs-block' ),
		sparkle: __( 'Sparkle', 'tabs-block' ),
		lightbulb: __( 'Lightbulb', 'tabs-block' ),
		users: __( 'People', 'tabs-block' ),
		chart: __( 'Chart', 'tabs-block' ),
		'check-circle': __( 'Check', 'tabs-block' ),
		globe: __( 'Globe', 'tabs-block' ),
		clock: __( 'Clock', 'tabs-block' ),
		layers: __( 'Layers', 'tabs-block' ),
		'shopping-bag': __( 'Shopping bag', 'tabs-block' ),
	};
 ...
 *
 * @param {string} name      Icon name.
 * @param {string} className Class applied to the `<svg>` element.
 * @return {JSX.Element|null} The icon, or null for `none` and unknown names.
 */
