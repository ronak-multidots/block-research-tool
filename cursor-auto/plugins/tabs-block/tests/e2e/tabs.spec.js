...
test.describe( 'Global Store Tabs', () => {
	...
	test( 'aligns the tab list left, center and right like core/tabs', async ( {
		editor,
		page,
	} ) => {
		await editor.insertBlock( { name: 'global-store/tabs' } );

		const block = editor.canvas.locator( '.wp-block-global-store-tabs' );

		await expect( block ).toHaveClass( /is-tabs-aligned-center/ );

		await page
			.getByRole( 'toolbar', { name: 'Block tools' } )
			.getByRole( 'button', { name: 'Align tabs left' } )
			.click();

		await expect( block ).toHaveClass( /is-tabs-aligned-start/ );

		await page
			.getByRole( 'toolbar', { name: 'Block tools' } )
			.getByRole( 'button', { name: 'Align tabs right' } )
			.click();

		await expect( block ).toHaveClass( /is-tabs-aligned-end/ );

		const [ saved ] = await editor.getBlocks();
		expect( saved.attributes.alignment ).toBe( 'end' );
	} );
} );
