...
final class Renderer {
	 ...
	 * @param string               $wrapper_attributes Pre-built wrapper attributes string. When null the
	 ...
	 *
	 * The markup is complete without JavaScript; `view.js` only keeps the digits ticking.
	 *
	 * @param array<string, mixed> $attrs            Sanitised attributes.
	...
	private static function render_countdown( array $attrs, ?int $remaining, int $expiry_timestamp, bool $has_expired ): string {
		...
		/*
		 * The digits update every second, which would flood assistive technology, so the
		 * visual timer is hidden from it and a throttled live region is announced instead.
		 */
		$markup .= sprintf(
			...
			// Self-contained rather than the theme-provided `screen-reader-text`.
	...
	private static function render_media( array $attrs ): string {
		...
		// A cutout with no alternative text is decorative, so hide the whole column.
	 ...
	 *
	 * The front-end script re-uses the same template so the announcement stays
	 * translated without shipping the i18n runtime to visitors.
	 *
	 * @return string
	 */
	 ...
	 *
	 * Pages served from a full-page cache carry a stale `data-server-now`, so the
	 * script needs a live source of truth before it trusts its own offset maths.
	 *
	 * @return string
	 */
	 ...
	 *
	 * Wrapped so tests can freeze time through the `gsfsh_current_time` filter.
	 *
	 * @return int
	 */
